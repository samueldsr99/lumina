export const jobs = [
  {
    id: 1,
    title: "Senior Software Engineer",
    location: "San Francisco, CA",
    description:
      "We are looking for a Senior Software Engineer to join our team in San Francisco, CA.",
    skills: [
      { name: "JavaScript", variant: "gray" },
      { name: "React", variant: "gray" },
      { name: "Node.js", variant: "gray" },
    ],
  },
  {
    id: 2,
    title: "Product Manager",
    location: "Remote",
    description:
      "We are looking for a Product Manager to join our team remotely.",
    skills: [
      { name: "Product Management", variant: "gray" },
      { name: "Agile", variant: "gray" },
      { name: "Scrum", variant: "gray" },
    ],
  },
  {
    id: 3,
    title: "UX Designer",
    location: "New York, NY",
    description:
      "We are looking for a Product Manager to join our team remotely.",

    skills: [
      { name: "User Experience", variant: "gray" },
      { name: "User Interface", variant: "gray" },
      { name: "Design", variant: "gray" },
    ],
  },
  {
    id: 4,
    title: "Data Scientist",
    location: "Chicago, IL",
    description:
      "We are looking for a Product Manager to join our team remotely.",
    skills: [
      { name: "Data Science", variant: "gray" },
      { name: "Machine Learning", variant: "gray" },
      { name: "Python", variant: "gray" },
    ],
  },
  {
    id: 5,
    title: "Marketing Manager",
    location: "Los Angeles, CA",
    description:
      "We are looking for a Product Manager to join our team remotely.",
    skills: [
      { name: "Marketing", variant: "gray" },
      { name: "Social Media", variant: "gray" },
      { name: "SEO", variant: "gray" },
    ],
  },

  {
    id: 6,
    title: "Senior Software Engineer",
    location: "San Francisco, CA",
    description:
      "We are looking for a Product Manager to join our team remotely.",
    skills: [
      { name: "JavaScript", variant: "gray" },
      { name: "React", variant: "gray" },
      { name: "Node.js", variant: "gray" },
    ],
  },
  {
    id: 7,
    title: "Product Manager",
    location: "Remote",
    description:
      "We are looking for a Product Manager to join our team remotely.",
    skills: [
      { name: "Product Management", variant: "gray" },
      { name: "Agile", variant: "gray" },
      { name: "Scrum", variant: "gray" },
    ],
  },
  {
    id: 8,
    title: "UX Designer",
    location: "New York, NY",
    description:
      "We are looking for a Product Manager to join our team remotely.",
    skills: [
      { name: "User Experience", variant: "gray" },
      { name: "User Interface", variant: "gray" },
      { name: "Design", variant: "gray" },
    ],
  },
  {
    id: 9,
    title: "Data Scientist",
    location: "Chicago, IL",
    description:
      "We are looking for a Product Manager to join our team remotely.",
    skills: [
      { name: "Data Science", variant: "gray" },
      { name: "Machine Learning", variant: "gray" },
      { name: "Python", variant: "gray" },
    ],
  },
  {
    id: 10,
    title: "Marketing Manager",
    location: "Los Angeles, CA",
    description:
      "We are looking for a Product Manager to join our team remotely.",
    skills: [
      { name: "Marketing", variant: "gray" },
      { name: "Social Media", variant: "gray" },
      { name: "SEO", variant: "gray" },
    ],
  },
];
